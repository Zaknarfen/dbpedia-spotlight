...
object ComplementTypes {
  ...
  aTypeManager.testArrayLength(1, allLangsArray.length, LOG)
  ...
  aTypeManager.testArrayLength(1, fileIteratorArray.length, LOG)

  def main(args : Array[String]) {
    val inputNTFilePath = args(0)
    val procedureNumber = args(1)

    aTypeManager.rebuildInterlanguageLinksFile(inputNTFilePath)

    // Core methods for the types complement task
    procedureNumber match {
      case "1" => aTypeManager.compTypesWithOtherLanguages()
      case "2" => aTypeManager.compTypesWithFreebaseLocal()
      case "3" => aTypeManager.compTypesWithFreebaseRemote()
      case _ => println("Enter a valid process number. Usage: \nmvn scala:run -Dlauncher=ComplementTypes \"-DjavaOpts.Xmx=$JAVA_XMX\" \"-DaddArgs=path\\to\\interlanguage_links_same_as_LANG.nt|PROCESS_NUMBER\"\n\nProcess number:\n 1) Complement with other languages.\n2) Complement with Freebase locally (needs close to 20GB memory)\n3) Complement with Freebase remotely.")
    }
  }
}